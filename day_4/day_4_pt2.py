...
def grid_search(cleaned_pattern):
    ...
    for row_index, line in enumerate(cleaned_pattern):
        ...
        for col_index, symbols in enumerate(line):
            ...
            if symbols == '@':
                ...
                if found < 4:
                    rolls += 1
                    # modify
                    # modifying_pattern['row'].append(row_index)
                    # modifying_pattern['col'].append(col_index)
                    modifying_pattern.append((row_index, col_index))
        ...
        # for row, col in zip(modifying_pattern['row'], modifying_pattern['col']):
        for row, col in modifying_pattern:
            cleaned_pattern[row][col] = 'X'

        
    return rolls, cleaned_pattern

def cleanning(pattern):
    cleaned_pattern = []
    for i in pattern:
        cleaned_pattern.append(list(i.strip()))
...
def main():
    ...
    rolls = 1
    while rolls > 0:
        rolls, cleaned_pattern = grid_search(cleaned_pattern)
        total_rolls += rolls
        print(f"Round complete. Changes made: {rolls}")
# There are 8557 rols of paper
    print(f"There are {total_rolls} rols of paper")
